    ...
    \begin{enumerate}
        ...
        \begin{proof}
            ...
            \begin{proofenumerate}
                \stepcounter{enumi}
                \item \( U\subset Y \) \timplies \( \inv{f}(U) \) offen bezüglich \( \tilde{d} \) \timplies \( \inv{f}(U) \) offen bezüglich \( d \).
                \item Sei \( U\subset X \) offen bezüglich \( \tilde{d} \), also auch offen bezüglich \( d \) \timplies \( \inv{f}(U) \) offen in \( Y \).
            \end{proofenumerate}
...
\begin{beispiele*}
    \begin{itemize}
        ...
        \item \( \reals^n \) mit \( \norm{\cdot}_{\infty}=\norm{\cdot}_{\max} \),
        \begin{align*}
            \norm{x}_{\infty}=\max\Set{\abs{x_1},\dotsc,\abs{x_n}}.
        \end{align*}
        ...
        \item \( \stetigefunktionen(\interval{a}{b}) \) mit \( \norm{f}_{\infty}=\sup_{t\in \interval{a}{b}}\abs{f(t)} \). 
    \end{itemize}
\end{beispiele*}
\begin{lemma}
    Sei \( (V,\norm{\cdot}) \) normierter VR\@. Dann wird durch \( d(x,y)\definedas \norm{x-y} \) eine Metrik auf \( V \) definiert (\enquote{induziert}).
\end{lemma}
\begin{proof}
...
\end{notation*}
\begin{bemerkung*}
    Nicht jede Metrik auf einem Vektorraum wird von einer Norm induziert, denn induzierte Metriken erfüllen \( d(\lambda x,\lambda y)=\abs{\lambda}d(x,y) \). Die diskrete Metrik erfüllt das nicht.
\end{bemerkung*}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{align*}
            \implies &\forall x\in U\logicspace \exists \varepsilon>0\logicspace \sd \logicspace B_{\varepsilon}^1(x)\subset U\\
...
\begin{folgerung*}
    ...
    \begin{align*}
        \iff \exists C,\tilde{C} \logicspace \sd \tilde{C}\norm{x_2}\leq \norm{x_1}\leq C\norm{x_2},
    \end{align*}
...
\end{satz}
\begin{proof}
    Aufgrund der Transitivität genügt es die Äquivalenz einer beliebigen Norm \( \norm{\cdot} \) zu \( \norm{\cdot}_{\infty} \) zu beweisen.
    \begin{proofenumerate}
        \item \( \norm{\cdot}_{\infty} \) ist stärker als \( \norm{\cdot} \):
        Denn sei \( x=\sum_{j=1}^{n}x_j e_j\in \reals^n  \), \( e_j=(0,\dotsc, \underset{j \text{-te}}{1},\dotsc,0) \). Dann ist
        \begin{align*}
            \norm{x}=\norm{\sum x_j e_j}\explain{\triangle \text{und Homog.}}{\leq}\abs{x_j}\cdot\norm{e_j}\leq \norm{x}_{\infty}\underbrace{\sum_{j=1}^{n}\norm{e_j}}_{=C}
        \end{align*}
        \item \( \norm{\cdot}_{\infty} \) ist schwächer als \( \norm{\cdot} \):
        
        Betrachte \( M\definedas \Set{x\in \reals^n|\norm{x}_{\infty}=1} \) (Einheits\enquote{sphäre} bezüglich \( \norm{\cdot}_{\infty} \), also Rand des Einheitswürfels \includegraphics[height=\baselineskip]{figures/einheitswuerfel_rand}).

        \begin{behauptung*}
            \( f\maps M\to \reals \), \( x\mapsto \norm{x} \) ist stetig bezüglich \( \norm{\cdot}_{\infty} \).
        \end{behauptung*}
        \begin{subproof}
            \begin{align*}
                \abs{f(x)-f(y)}=\abs{\norm{x}-\norm{y}}\underset{(*)}{\leq}\norm{x-y}\leq C\norm{x-y}_{\infty}
            ...
            \begin{align*}
                ...
                \norm{y}-\norm{x}&=\norm{y+x-x}-\norm{x}\leq \norm{x+y}.
            \end{align*}
        \( M \) ist abgeschlossen bezüglich \( \norm{\cdot}_{\infty} \) (denn \( \reals^n\setminus M= \) Urbild der offenen Menge \( \reals\setminus \Set{1} \) unter der stetigen Abbildung \( x\mapsto \norm{\cdot}_{\infty} \)). \( M\subset  \) abgeschlossenen \emph{Quader} und dieser ist \emph{kompakt} in \( (\reals^n,\norm{\cdot}_{\infty}) \) (\thref{quader_ist_kompakt_norm_undendlich}) \timplies \( M \) ist kompakt (\ref{abgeschlossene_teilmenge_kompakter_menge_ist_kompakt}).

        Es folgt: \( f \) nimmt sein Minimum \( b \) an und (da \( f>0 \)) somit ist \( b>0 \). Nach Definition ist \( \norm{y}\geq b\quad \forall y\in M \). Für alle \( x\in \reals^n\setminus\Set{0} \) gilt \( \frac{x}{\norm{x}_{\infty}}\in M \), also ist \( \norm{\frac{x}{\norm{x}_{\infty}}}\geq b \), also \( \norm{x}\geq b\norm{x}_{\infty} \) und für \( x=0 \) gilt dies ohnehin.
        \end{subproof}
    \end{proofenumerate}
\end{proof}
\begin{lemma}\label{quader_ist_kompakt_norm_undendlich}
    Der Quader \( Q=\Set{x=(x_1,\dotsc, x_n)\in \reals^n| a_j\leq x_j\leq b_j} \) ist kompakt in \( \reals^n,\norm{\cdot}_{\infty} \) (\( a_j\leq b_j \)).
\end{lemma}
\begin{proof}
    ...
    \begin{eigenschaftenenumerate}
        ...
        \item \( \diam Q_m=2^{-m}\diam Q \).
    \end{eigenschaftenenumerate}
    \minisec{Beachte:} \( \diam Q= \) Länge der länsten Seite bezüglich \( \norm{\cdot}_{\infty} \).
    \begin{figure}[H]
        \centering
    ...
    \begin{align*}
        \diam(Q_{m+1})=\frac{1}{2}\diam(Q_m)=\frac{1}{2^{m+1}}\diam(Q).
    \end{align*}
    
    Nach dem Schachtelungsprinzip \texists \( a\in Q_m \) \tforall \( m \). Da \( (U_j)_j \) \( Q \) überdeckt \texists \( U_{j_0} \) \sd \( a\in U_{j_0} \). \( U_{j_0} \) offen \timplies \texists \( \varepsilon>0 \) \sd \( B_{\varepsilon}^{\norm{\cdot}_{\infty}}(a)\subset U_{j_0} \). Wähle \( m \) so groß, dass \( \diam(Q_m)<\varepsilon \). \( a\in Q_m \) \timplies \( Q_m\subset B_{\varepsilon}^{\norm{\cdot}_{\infty}}(a)\subset U_{j_0} \) \contra Widerspruch Konstruktion der \( Q_m \).
\end{proof}
\begin{bemerkung}
...
\begin{bemerkungen}
    ...
    \begin{enumerate}
        ...
        \( (\stetigefunktionen(\interval{a}{b}), \norm{\cdot}_{\infty}) \) ist vollständig, \( (\stetigefunktionen(\interval{a}{b}), \norm{\cdot}_{L^1}) \) nicht.
...
\begin{proof}
    \begin{proofdescription}
        ...
        \item[\rueck] It \( A \) beschränkt so ist \( A \) in einem Quader enthalten (denn \( \norm{x-y}_{\infty}\leq  \norm{x-y} \) somit \( \diam_{\norm{\cdot}_{\infty}}(A)\leq C\diam_{\norm{\cdot}}(A)<\infty \)). \( Q \) ist kompakt (bezüglich \( \norm{\cdot}_{\infty} \) somit bezüglich \( \norm{\cdot} \)).\( A \) abgeschlossen \timplies \( A \) kompakt (\ref{abgeschlossene_teilmenge_kompakter_menge_ist_kompakt}).
    \end{proofdescription}
\end{proof}
\begin{bemerkung*}
