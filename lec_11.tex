...
\begin{proof}
  ...
  \end{proofenumerate}
\end{proof}
\begin{satz}[Taylorsche Formel]\index[theorems]{Taylorsche Formel}
...
\begin{lemma}[aus der AGLA\@: Kriterium von Hurwitz]
  ...
  \begin{equation*}
    \determinant{\explain{\text{Haupt-Minoren von \( A \)}}{\begin{pNiceMatrix} a_{11} & \Cdots & a_{1k} \\ \Vdots &  & \Vdots \\ a_{k1} & \Cdots & a_{kk} \end{pNiceMatrix}}}>0\quad \forall k\in \set{1,\dotsc,n}
  \end{equation*}
  und genau dann negativ definit, wenn
  \begin{align*}
    \determinant+{a_{11}}=a_{11}&<0\\
    \determinant{\begin{pNiceMatrix} a_{11} & a_{12} \\ a_{21} & a_{22} \end{pNiceMatrix}}&>0
  \end{align*}
